...
def agg_visualize(df: pd.DataFrame, subjects: list, save_path: str = 'analysis/figures') -> bool:
    ...
    df_smart = df[df['folder'] == 'raw']
    df_auxi_db = df[df['folder'] == 'auxi_db']
    df_improved = df[df['folder'] == 'auxi_logos_extract']
    ...
    ax.errorbar(x - 0.2, df_smart['median_accuracy'], 
                yerr=[df_smart['median_accuracy'] - df_smart['min_accuracy'], 
                      df_smart['max_accuracy'] - df_smart['median_accuracy']], 
                fmt='o', color='orange', ecolor='black', label='Raw', capsize=5)

    # Plot error bars for df_auxi_db
    ax.errorbar(x, df_auxi_db['median_accuracy'],
                yerr=[df_auxi_db['median_accuracy'] - df_auxi_db['min_accuracy'],
                        df_auxi_db['max_accuracy'] - df_auxi_db['median_accuracy']],
                fmt='o', color='blue', ecolor='black', label='Auxi_db', capsize=5)

    # Plot error bars for df_improved
    ax.errorbar(x + 0.2, df_improved['median_accuracy'], 
                yerr=[df_improved['median_accuracy'] - df_improved['min_accuracy'], 
                      df_improved['max_accuracy'] - df_improved['median_accuracy']], 
                fmt='o', color='red', ecolor='black', label='Auxi_logos_extract', capsize=5)
   
    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_xlabel('Subjects')
    ax.set_ylabel('Accuracy')
    ax.set_title('Accuracy by subject and folder | Humanities')
    ax.set_xticks(x)
    ax.set_xticklabels(df_smart['topic'])
    ax.legend()

    # Save to file
    filename = 'humanities_accuracy_+auxidb.png'
    plt.savefig(os.path.join(save_path, filename))

    print(f'Finished aggregate visualizations. Save figure to {os.path.join(save_path, filename)}')
    return True
...
def main() -> None:
    ...
    for sub in subjects:
        visualize(aggregated_stats_df, sub, save_path='test/auxi_logos_extract/figures')

    agg_visualize(aggregated_stats_df, subjects, save_path='test/auxi_logos_extract/figures')
